...
const PHASE_NAMES = ['new', 'q1', 'full', 'q3']

function phase_range (start, end) {
  start = start.getTime()
  end = end.getTime()
  ...
  {
    const d = new Date(t)
    k = 4 * Math.floor(12.3685 * (d.getFullYear() + (1.0 / 12.0) * d.getMonth() - 1900))
  }

  const list = []
  while (1) {
    const date = truephase(k >> 2, k)
    if (date.getTime() > end) {
      break
    }
    if (date.getTime() >= start) {
      list.push({phase: PHASE_NAMES[k & PHASE_MASK], date: date})
    }
    k++
  }
  return list
