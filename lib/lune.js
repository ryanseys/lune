...
function phaseRange (start, end, phase) {
  if (!(start && start instanceof Date)) {
    throw new Error('Invalid parameter. First argument must be a Date.');
  } else if (start.toString() === 'Invalid Date') {
    throw new Error('Invalid date. Second argument not a valid date.');
  }
  
  if (!(end && end instanceof Date)) {
    throw new Error('Invalid parameter. Second argument must be a Date.');
