...
function phase (phase_date) {
  ...
  const sun_mean_anomaly = fixangle((360 / 365.2422) * day + ECLIPTIC_LONGITUDE_EPOCH - ECLIPTIC_LONGITUDE_PERIGEE)

  // Solve Kepler's equation
  // True anomaly
  const sun_true_anomaly = 2 * todeg(Math.atan(Math.sqrt((1 + ECCENTRICITY) / (1 - ECCENTRICITY)) * Math.tan(0.5 * kepler(sun_mean_anomaly, ECCENTRICITY))))

  // Suns's geometric ecliptic longuitude
  const sun_ecliptic_longitude = fixangle(sun_true_anomaly + ECLIPTIC_LONGITUDE_PERIGEE)
  ...
  const evection = 1.2739 * dsin(2 * (moon_longitude - sun_ecliptic_longitude) - MM)

  // Annual equation
  const annual_eq = 0.1858 * dsin(sun_mean_anomaly)

  // Correction term
  // XXX: capital vars should only be used for local constants
  const A3 = 0.37 * dsin(sun_mean_anomaly)
  ...
  // Variation
  const variation = 0.6583 * dsin(2 * (lP - sun_ecliptic_longitude))
  ...
  const moon_age = fixangle(lPP - sun_ecliptic_longitude)

  // Calculate distance of Moon from the centre of the Earth
  const moon_distance = (MOON_SMAXIS * (1 - MOON_ECCENTRICITY * MOON_ECCENTRICITY)) / (1 + MOON_ECCENTRICITY * dcos(MmP + mEc))
  ...
  return {
    ...
    angular_diameter: MOON_ANGULAR_SIZE * MOON_SMAXIS / moon_distance,
